...
exports.getMatRung = async (req, res, next) => {
  try {
    ...
    // No filters - return 12 months data (changed from 3 months to handle older data)
    if (!fromDate && !toDate && !huyen && !xa && !tk && !khoanh && !churung) {
      ...
      const query = `
        SELECT
          ...
          ${hasUsers ? `
          -- User info (from gateway headers or join)
    ...
    // Only add spatial filters if table exists
    if (hasRanhGioiHC) {
      if (huyen) {
    ...
    // Check if churung table exists
    const hasRg3lr = churung ? await tableExists(db, 'laocai_rg3lr') : false;
    let churungJoin = '';
    if (churung && hasRg3lr) {
    ...
    const query = `
      SELECT
        ...
        NULL as khoanh,
        `}
        ${(churung && hasRg3lr) ? 't.churung,' : 'NULL as churung,'}
  ...
  try {
    ...
    const query = `
      SELECT
        ...
        ${hasRanhGioiHC ? `COUNT(CASE WHEN r.gid IS NOT NULL THEN 1 END) as records_with_spatial_data,` : `0 as records_with_spatial_data,`}
